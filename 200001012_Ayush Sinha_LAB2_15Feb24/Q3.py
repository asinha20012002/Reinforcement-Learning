...
def get_valid_actions(state):
    actions = []
    if state[0] > 0 and maze[state[0] - 1, state[1]] != -1:  # Up
        actions.append(0)
    if state[0] < maze.shape[0] - 1 and maze[state[0] + 1, state[1]] != -1:  # Down
        actions.append(1)
    if state[1] > 0 and maze[state[0], state[1] - 1] != -1:  # Left
        actions.append(2)
    if state[1] < maze.shape[1] - 1 and maze[state[0], state[1] + 1] != -1:  # Right
        actions.append(3)
    return actions
